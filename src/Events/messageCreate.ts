import { type Client, type Message, PermissionFlagsBits } from "discord.js";
...
class MessageCreateEvent {
	...
	private async handleMessage(): Promise<void> {
		...
		console.log(
			`[🔄 COMMANDS] Running command: ${command} By ${this.messageCreate.author.username}`,
		);
		cmd.run(
			this.client,
	...
	private async handleEnderbotMessage(): Promise<boolean> {
		if (
			this.messageCreate.author.bot &&
			this.messageCreate.author.id === MessageCreateEvent.enderbotId
		) {
	...
	private getCommand(command: string): Command {
		return ApplicationInstance.CommandsList.has(command)
			? ApplicationInstance.CommandsList.get(command)
			: ApplicationInstance.AliasesList.has(command)
				? ApplicationInstance.CommandsList.get(
						ApplicationInstance.AliasesList.get(command),
					)
				: ApplicationInstance.CommandsList.get(command);
	}
}
