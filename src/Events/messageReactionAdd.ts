...
import EnderbotParser, {
	type EnderbotCraftData,
	type EnderbotForgeData,
} from "../Modules/Enderbot/EnderbotParser";
...
class messageReactionAdd {
	...
	public async collector(): Promise<void> {
		...
		if (this.isEnderbotForge()) {
			const forge = EnderbotParser.parseForgeEmbed(
				this.message.message as Message,
			);
		...
		if (this.isInventoryAdd()) {
			const inventory = EnderbotParser.parseInventoryEmbed(
				this.message.message as Message,
			);
			if (!inventory) return;
			const args = Object.keys(inventory).map(
				(key) => `${inventory[key].underscoreId} ${inventory[key].number}`,
			);
			await Add.run(
	...
	private async handleEnderbotAction(
		action: EnderbotCraftData | EnderbotForgeData,
		message: Message,
		userId: string,
	): Promise<void> {
		...
		const costArr = Object.entries(action.cost)
			...
			.map(([key, value]) => {
				...
				return `${value.underscoreId} ${Number.parseInt(`${nb}`)}`;
			});
