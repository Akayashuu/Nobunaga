...
export type { AutcompleteOptions };
...
class Autocomplete {
	 ...
	 * @type {number}
	 */
	private common_substring_start_index = 1;
	/**
	...
	constructor(
		...
		str_input: string,
		options: AutcompleteOptions = {},
	) {
		this.availableOptions = availableOptions;
		this.str_input = str_input.toLowerCase();
		this.common_substring_start_index =
			options.common_substring_start_index || 1;
	...
	public getCommandAutocomplete(): string {
		for (const option of this.availableOptions) {
			if (
				option.toLowerCase().startsWith(this.str_input) ||
				option.toLowerCase() === this.str_input
			)
	...
	public getSubArgumentAutocomplete(): string | false {
		...
		for (const option of this.availableOptions) {
			if (
				this.str_input.startsWith(option.toLowerCase()) ||
				option.toLowerCase() === this.str_input
			)
				return option.toLowerCase();
		}
		if (this.hasToBeExact && !this.availableOptions.includes(this.str_input))
			return false;
		let highestCommonSubstring_number = 0,
			key = "";
		for (const k of this.availableOptions) {
			for (
				let i = this.common_substring_start_index;
				i <= this.str_input.length;
				i++
			) {
				const commonSubstrings = this.findCommonSubstrings(
					this.str_input,
					k.toLowerCase(),
					i,
				);
				if (commonSubstrings.length >= 1) {
					for (const substring of commonSubstrings) {
						if (
							substring.length > highestCommonSubstring_number ||
							k.startsWith(this.str_input)
						) {
		...
		let bestMatch = { option: "", distance: Number.POSITIVE_INFINITY };
		for (const k of this.availableOptions) {
			const currentDistance = new levenshtein(this.str_input, k.toLowerCase())
				.distance;
			if (currentDistance < bestMatch.distance) {
				bestMatch = { option: k.toLowerCase(), distance: currentDistance };
			}
		}
		if (this.availableOptions.includes(this.str_input)) {
	...
	public getSubArgumentAutocompleteWithKey(): string | false {
		...
		for (const option of this.availableOptions) {
			if (
				...
				return this.availableOptionsKeys[this.availableOptions.indexOf(option)];
		}
		let highestCommonSubstring_number = 0,
			key = "";
		for (const k of this.availableOptions) {
		...
		if (highestCommonSubstring_number > 0)
			return this.availableOptionsKeys[this.availableOptions.indexOf(key)];
		let bestMatch = { option: "", distance: Number.POSITIVE_INFINITY };
		for (const k of this.availableOptions) {
			const currentDistance = new levenshtein(this.str_input, k.toLowerCase())
				.distance;
		...
		if (this.availableOptions.includes(this.str_input)) {
			return this.availableOptionsKeys[
				this.availableOptions.indexOf(this.str_input)
			];
		}
		if (bestMatch.distance <= 2) {
			return this.availableOptionsKeys[
				this.availableOptions.indexOf(bestMatch.option)
			];
		}
		return false;
	}
